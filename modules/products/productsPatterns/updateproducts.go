...
type updateProductBuilder struct {
	...
	filesUsecases  filesusecase.IFileUsecase
	query          string
	queryFields    []string
	lastStackIndex int
	values         []any
...
func NewUpdateProductBuilder(db *sqlx.DB, req *products.Products, filesUsecases filesusecase.IFileUsecase) IUpdateProductBuilder {
...
func NewUpdateProductEngineer(b IUpdateProductBuilder) *updateProductEngineer {
...
func (b *updateProductBuilder) deleteOldImages() error {
	...
	if len(images) > 0 {
		...
		b.filesUsecases.DeleteImageLocal(deleteFileReq)
	}
